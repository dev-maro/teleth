...
VIP_ALPOP = (5426390871, 5176749470, 2095357462)
VIP_ALPOP = (925972505, 1895219306, 6269975462, 1985225531)
...
async def ge(user, event):
    ...
    try:
        ...
        return None
    return user_obj
########################  ZThon Userbot ~ By: Zelzal (@VIP_ALPOP)  ########################
...
async def ahmed(ahmed): # Code Update by @VIP_ALPOP
    zelzal = ahmed.pattern_match.group(1)
    zilzal = await ahmed.get_reply_message()
    zel_zal = random.choice(kettuet)
    if not zilzal and not zelzal: # Code Update by @VIP_ALPOP
        return await edit_or_reply(ahmed, f"**⌔╎{zel_zal}**")
    user, custom = await get_user_from_event(ahmed)
    if not user: # Code Update by @VIP_ALPOP
        return
    VIP_ALPOP2 = user.first_name.replace("\u2060", "") if user.first_name else user.username
    if custom: # Code Update by @VIP_ALPOP
        VIP_ALPOP2 = custom
    me = await ahmed.client.get_me()
    my_first = me.first_name
    my_ahmed = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(ahmed, f"**⌔╎لـ  ** [{VIP_ALPOP2}](tg://user?id={user.id}) \n**⌔╎{zel_zal}**")
...
async def zed(malath):
    ...
    if not user:
        return
    VIP_ALPOP = user.first_name.replace("\u2060", "") if user.first_name else user.username
    zedt = random.choice(mth)
    await edit_or_reply(malath, f"**✾╎نـسبـة حبكـم انـت و**  [{VIP_ALPOP}](tg://user?id={user.id}) **هـي {zedt} 😻♥️**")
...
async def zed(malath):
    ...
    await edit_or_reply(malath, f"**✾╎نسبـة الانوثه لـ**  [{VIP_ALPOP}](tg://user?id={user.id}) **هـي {zedt} 🤰**")
...
async def permalink(malath):
    ...
    VIP_ALPOP = user.first_name.replace("\u2060", "") if user.first_name else user.username
    zedt = random.choice(zid)
    await edit_or_reply(malath, f"**✾╎نسبـة الغبـاء لـ**  [{VIP_ALPOP}](tg://user?id={user.id}) **هـي {zedt} 😂💔**")
...
async def permalink(malath):
    ...
    VIP_ALPOP = user.first_name.replace("\u2060", "") if user.first_name else user.username
    zedt = random.choice(zid)
    await edit_or_reply(malath, f"**✾╎نسبـة الانحـراف لـ**  [{VIP_ALPOP}](tg://user?id={user.id}) **هـي {zedt} 🥵🖤**")
...
async def permalink(malath):
    ...
    VIP_ALPOP = user.first_name.replace("\u2060", "") if user.first_name else user.username
    zedt = random.choice(zid)
    await edit_or_reply(malath, f"**✾╎نسبـة المثليـه لـ**  [{VIP_ALPOP}](tg://user?id={user.id}) **هـي {zedt} 🤡 🏳️‍🌈.**")
...
async def permalink(malath):
    ...
    VIP_ALPOP = user.first_name.replace("\u2060", "") if user.first_name else user.username
    zedt = random.choice(zid)
    await edit_or_reply(malath, f"**✾╎نسبـة النجـاح لـ** [{VIP_ALPOP}](tg://user?id={user.id}) **هـي {zedt} 🤓.**") 
...
async def permalink(malath):
    ...
    VIP_ALPOP = user.first_name.replace("\u2060", "") if user.first_name else user.username
    zedt = random.choice(zid)
    await edit_or_reply(malath, f"**✾╎نسبـة الكراهيـة لـ** [{VIP_ALPOP}](tg://user?id={user.id}) **هـي {zedt} 🤮.**")
...
async def zed(malath):
    ...
    if user.id in VIP_ALPOP:
        return await edit_or_reply(malath, f"**╮ ❐ عـذراً .. لا يمكننـي اهانـة مطـور السـورس  ❏╰**")
    VIP_ALPOP2 = user.first_name.replace("\u2060", "") if user.first_name else user.username
    await edit_or_reply(malath, f"**🚹 ╎ المستخـدم  ⪼ • ** [{VIP_ALPOP2}](tg://user?id={user.id}) \n☑️ **╎تم رفعـه ورع القـروب 😹🙇🏻.**")
...
async def zed(malath):
    ...
    if user.id in VIP_ALPOP:
        return await edit_or_reply(malath, f"**╮ ❐ عـذراً .. لا يمكننـي اهانـة مطـور السـورس  ❏╰**")
    VIP_ALPOP2 = user.first_name.replace("\u2060", "") if user.first_name else user.username
    await edit_or_reply(malath, f"**🚺 ╎ الحلـوه ⪼ • ** [{VIP_ALPOP2}](tg://user?id={user.id}) \n☑️ **╎ تم رفعـها مـزة الجروب  🥳💃.**")

@zedub.zed_cmd(pattern="رفع علق(?: |$)(.*)")
async def zed(malath):
    user, custom = await get_user_from_event(malath)
    if not user:
        return
...
async def permalink(malath):
    ...
    VIP_ALPOP2 = user.first_name.replace("\u2060", "") if user.first_name else user.username
    await edit_or_reply(
        malath, f"**🚹 ╎ المستخـدم  ⪼ • ** [{VIP_ALPOP2}](tg://user?id={user.id}) \n☑️ **🐈╎ تم رفعـه قطه .** \n"
    )
...
async def zed(malath):
    ...
    if not user:
        return
    VIP_ALPOP2 = user.first_name.replace("\u2060", "") if user.first_name else user.username
    await edit_or_reply(malath, f"**🚹 ╎ الحلـو  ⪼ • ** [{VIP_ALPOP2}](tg://user?id={user.id}) \n☑️ **╎ تم رفعـه حبيبـج .. بنجـاح 💍🤵‍♂👰🏻‍♀.**")
...
@zedub.zed_cmd(pattern="رفع شرموط(?: |$)(.*)")
async def zed(malath):
    user, custom = await get_user_from_event(malath)
    if not user:
        return
...
@zedub.zed_cmd(pattern="رفع شرموطه(?: |$)(.*)")
async def zed(malath):
    user, custom = await get_user_from_event(malath)
    if not user:
        return
    VIP_ALPOP2 = user.first_name.replace("\u2060", "") if user.first_name else user.username
    await edit_or_reply(malath, f"**🚹 ╎ الحلـوه  ⪼ • ** [{VIP_ALPOP2}](tg://user?id={user.id}) \n☑️ **╎  تم رفعـها شرموطه الجروب  👸🏼.**")
...
async def permalink(malath):
    ...
    if not user:
        return
    if user.id in VIP_ALPOP:
        return await edit_or_reply(malath, f"**╮ ❐ لك هـذا مطـور . . شما حجيت معرفش اوصفـه 🙊💘 ❏╰**")
    if user.id in VIP_ALPOP:
        return await edit_or_reply(malath, f"**╮ ❐ لك هـذا البوب  . . شما حجيت معرفش اوصفـه 🙊💘 ❏╰**")
    tag = user.first_name.replace("\u2060", "") if user.first_name else user.username
