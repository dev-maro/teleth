...
async def _(event):
    ...
    sandy = await edit_or_reply(
        ...
        f"**•⎆┊يتـم الان اعـادة تشغيـل بـوت البوب  قـد يستغـرق الامـر 1-2 دقيقـه ▬▭ ...**",
    )
    try:
...
@zedub.zed_cmd(
    ...
    info={
        ...
        "الوصـف": "لـ إطفـاء الداينـو الخاص بتنصيبك بهيروكـو .. لا تستطيع اعاده التشغيل مرة اخرى عبر حسابك عليك الذهاب لحساب هيروكو واتباع الشرح التالي https://t.me/SOURCE_ALPOP",
...
async def _(event):
    ...
    o = stdout.decode()
    OUTPUT = (
        f"**[𝐒𝐎𝐔𝐑𝐂𝐄 𝐀𝐋𝐏𝐎𝐏](tg://need_update_for_some_feature/) - فـارات تنصيبـك هـي:**\n\n\n{o}"
    )
    if len(OUTPUT) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            out_file.name = "env.text"
