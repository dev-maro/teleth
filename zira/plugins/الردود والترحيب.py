...
ZelzalWF_cmd = (
    "𓆩 [𝐒𝐎𝐔𝐑𝐂𝐄 𝐀𝐋𝐏𝐎𝐏 - اوامـر الـردود / والتـرحيب](t.me/SOURCE_ALPOP) 𓆪\n\n"
    "**✾╎قائـمه اوامـر الـردود 🦾 :** \n\n"
    "**⎞𝟏⎝** `.رد`\n"
    "**•• ⦇الامـر + اسـم الـرد بالـرد ع كلمـة الـرد او بالـرد ع ميديـا⦈ لـ اضـافة رد بالجروب **\n\n"
    "**⎞𝟐⎝** `.حذف رد`\n"
...
@zedub.zed_cmd(
    ...
    info={
        ...
        "اضـافـات الــرد": {
            "{mention}": "اضافه منشن",
            "{title}": "اضافة اسم جروب  الـرد",
            "{count}": "اضافة عدد اعضاء الجروب ",
...
async def add_new_filter(event):
    ...
    if msg and msg.media and not string:
        if BOTLOG:
            ...
            await edit_or_reply(
                event,
                "**❈╎يتطلب رد ميديـا تعيين جروب  السجـل اولاً ..**\n**❈╎لاضافـة جروب  السجـل**\n**❈╎اتبـع الشـرح ⇚** https://t.me/zzzvrr/13",
            )
            return
...
async def on_snip_list(event):
    ...
    for filt in filters:
        if OUT_STR == "** ❈╎لاتوجـد ردود محفوظـه في هـذه الدردشـه ༗**":
            OUT_STR = "𓆩 𝐒𝐎𝐔𝐑𝐂𝐄 𝐀𝐋𝐏𝐎𝐏 - قائمـة الـردود 𓆪\n⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n"
        OUT_STR += "👉 `{}`\n".format(filt.keyword)
    await edit_or_reply(
        event,
        OUT_STR,
...
async def _(event):
    ...
    if gvarstatus("TIME_STOP") is not None: #Code by T.me/VIP_ALPOP
    ...
    if msg and msg.media and not string:
        if BOTLOG_CHATID:
            ...
            return await edit_or_reply(
                event,
                "**يتطلب حفظ تـرحيب الميـديـا .. تعيين فـار جروب  السجـل ؟!...**",
            )
