    ...
    for i in range(sandy):
        try:
            ...
            await event.client.send_message(
                BOTLOG_CHATID,
                f"**⎉╎عــذراً .. لـيس لـديــك صـلاحيـات فـي ↠**\n**⎉╎جروب :** {get_display_name(achat)}(`{achat.id}`)",
            )
    end = datetime.now()
    ...
    if BOTLOG and count != 0:
        if reason:
            await event.client.send_message(
                ...
                \n**- الايــدي : **`{user.id}`\
                \n**- الســبب :** `{reason}`\
                \n**- تـم الغــاء حظـره مـن  {count} جروب**\
                \n**- الــوقت المسـتغــࢪق :** {zedtaken} **ثــانيـه**",
            )
        else:
            await event.client.send_message(
...
@zedub.zed_cmd(
    ...
    info={
        "header": "لـ تثبيـت الرسـائـل فـي الجروب ",
...
async def pin(event):
    "لـ تثبيـت الرسـائـل فـي الجروب "
...
@zedub.zed_cmd(
    ...
    info={
        "header": "لـ الغــاء تثبيـت الرسـائـل فـي الجروب ",
        "امـر مضـاف": {"الكل": "لـ الغــاء تثبيـت كــل الرسـائـل فـي الجروب "},
        "الاسـتخـدام": [
...
async def unpin(event):
    "لـ الغــاء تثبيـت الرسـائـل فـي الجروب "
...
async def _iundlt(event):  # sourcery no-metrics
    ...
    deleted_msg = f"**- اليـك آخـر {lim} رسـائـل محذوفــه لـ هـذا الجروب 🗑 :**"
    if not flag:
        for msg in adminlog:
