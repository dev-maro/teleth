...
async def get_pack_info(event):
    ...
    OUTPUT = (
        f"ᯓ 𝐒𝐎𝐔𝐑𝐂𝐄 𝐀𝐋𝐏𝐎𝐏 𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - معلومـات الملصـق 🎟\n"
        f"⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n"
...
async def cb_sticker(event):
    split = event.pattern_match.group(1) #Code by T.me/VIP_ALPOP
    ...
    reply = f"ᯓ 𝐒𝐎𝐔𝐑𝐂𝐄 𝐀𝐋𝐏𝐎𝐏 𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - بحث الملصقـات 🎟\n⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n**- نتائـج البحث عـن حـزم ملصقـات بـ اسـم {split} :**"
    for pack in results:
        if pack.button:
...
async def zelzal_gif(event):
    ...
    chat = "@Stickers" #Code by T.me/VIP_ALPOP
    zed = await edit_or_reply(event, "**⎉╎جـارِ حـذف الملصـق مـن حزمتـك ...**")
    async with borg.conversation(chat) as conv: #Code by T.me/VIP_ALPOP
        try:
            await conv.send_message("/start")
            await conv.get_response()
            await conv.send_message("/delsticker")
            await conv.get_response()
            await conv.send_file(reply) #Code by T.me/VIP_ALPOP
            await conv.get_response()
