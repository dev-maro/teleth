...
@zedub.zed_cmd(
    pattern="انشاء (جروب |خارق|قناه) ([\s\S]*)",
    command=("انشاء", plugin_category),
    info={
        "header": "لـ إنشـاء (جروب  خارق/جروب /قناه) باستخـدام البـوت",
        "امر اضافي": {
            "خارق": "لـ إنشـاء مجمـوعـة خـارقـه",
            "جروب ": "لـ إنشـاء مجمـوعـة",
            "قناه": "لـ إنشـاء قنـاة",
        },
        "الاستخـدام": "{tr}انشاء (خارق/جروب /قناه) + اسـم (القنـاة/الجروب )",
        "مثــال": "{tr}انشاء قناه البوب ",
    },
)
async def _(event):
    "لـ إنشـاء (جروب  خارق/جروب /قناه) باستخـدام البـوت"
    type_of_group = event.pattern_match.group(1)
    group_name = event.pattern_match.group(2)
    if type_of_group == "قناه":
        descript = "**⎉╎هـذه القنـاة تم إنشائهـا بواسطـة .. البوب ™**"
    else:
        descript = "**⎉╎هـذا المجمـوعـه تم إنشائهـا بواسطـة .. البوب ™**"
    if type_of_group == "جروب ":
        try:
            ...
            await edit_or_reply(
                event, f"**⎉╎الجروب  `{group_name}` تم إنشـائه.. بنجـاح✓** \n**⎉╎الرابـط** {result.link}"
            )
        ...
        if answer[0] != "error":
            await edit_or_reply(
                event,
                f"**⎉╎الجروب  الخـارق `{group_name}` تم إنشـائه.. بنجـاح✓** \n**⎉╎الرابـط** {answer[0].link}",
            )
        else:
            await edit_delete(event, f"**- خطـأ :**\n{answer[1]}")
    else:
