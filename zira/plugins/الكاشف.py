...
ZelzalPH_cmd = (
    "𓆩 [𝐒𝐎𝐔𝐑𝐂𝐄 𝐀𝐋𝐏𝐎𝐏 📲 - كـاشـف الارقـام العربيــة](t.me/SOURCE_ALPOP) 𓆪\n\n"
    "**⪼ الامــر ↵**\n\n"
...
async def _(event): #Code by T.me/VIP_ALPOP
    ...
    zelzal = "@SOURCE_ALPOP_BOT" #Code by T.me/VIP_ALPOP
    zed = await edit_or_reply(event, "**⎉╎جـارِ الكشـف عن الرقـم 📲**\n**⎉╎انتظـر قليـلاً ... ▬▭**")
    async with borg.conversation(zelzal) as conv: # code by T.me/VIP_ALPOP
        try:
            ...
            await zedub(unblock("@SOURCE_ALPOP_BOT"))
            await conv.send_message("/start")
            await conv.get_response()
            await conv.send_message(number)
            zthon = await conv.get_response()
